#ifndef ROLLCRITICAL_H
#define ROLLCRITICAL_H
...
static USHORT g_criticalKeyScanCode = 19;  // Default R key scan code
static USHORT g_dodgeKeyScanCode = 16;     // Default Q key scan code
static int g_criticalKeyMouseButton = 0;   // 0 = keyboard, 1-5 = mouse button

// Send mouse button click
void SendMouseButton(int buttonIndex) {
    INPUT input[2] = {};
...
// Send critical attack key (quick tap)
void SendCriticalKey() {
    if (g_criticalKeyMouseButton != 0) {
        SendMouseButton(g_criticalKeyMouseButton);
        return;
    }

    if (g_criticalKeyScanCode == 0) return;

    INPUT input[2] = {};
    input[0].type = INPUT_KEYBOARD;
    input[0].ki.wVk = 0;
    input[0].ki.wScan = g_criticalKeyScanCode;
    input[0].ki.dwFlags = KEYEVENTF_SCANCODE;

    input[1].type = INPUT_KEYBOARD;
    input[1].ki.wVk = 0;
    input[1].ki.wScan = g_criticalKeyScanCode;
...
void SetCriticalKeyScanCode(USHORT sc) {
    g_criticalKeyScanCode = sc;
}

void SetCriticalKeyMouseButton(int buttonIndex) {
    g_criticalKeyMouseButton = buttonIndex;
}
...
// Trigger roll critical action (called by main.cpp)
void TriggerRollCritical() {
    if (g_enabled) {
...
// Initialize the RollCritical system
void StartRollCritical() {
...
// Stop the RollCritical system
void StopRollCritical() {
...
bool IsRollCriticalRunning() {
    return g_running;
}

} // namespace RollCritical

#endif // ROLLCRITICAL_H
